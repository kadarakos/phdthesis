...
The translation model decoder is an attention-based recurrent neural network \cite{Bahdanau2015}. Tokens in the decoder are 
...
The image prediction decoder is trained to predict the visual feature vector of the image associated with a sentence \cite{Chrupala2015}. 
...
$\alpha$, and the cosine distance $d(\cdot, \cdot)$. A margin-based objective has previously been used in grounded representation learning \cite{Vendrov2016,Chrupala2017}. The contrastive examples $\mathbf{v'}$ are drawn from the other instances in a minibatch:
%
\begin{align}
	\begin{split}
