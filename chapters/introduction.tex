...
\textbf{meaning representation bridging between languages}.
The methods presented in this thesis seek to find relationships between \textbf{images} and
\textbf{natural utterances} in \emph{multiple languages}.
...
interpretations from a linguistic perspective using the black-box architecture that is the basis
...
\emph{transfer-learning} where we seek to identify unsupervised learning objectives,
...
To aid the reader with technical and historical context
...
Furthermore, consider the applicability of distributional language representations in the larger scope of Artificial
...
and Section~\ref{sec:trans-sentence} situates the reader in the area. Most of these approaches to date are
...
also provides a natural bridge between various languages. Linguistic utterances that are similar
...
can be conceptualized as \emph{multiple-views} of the same underlying \emph{ideal datapoint}.
...
it provides orthogonal improvements to having extra parallel English-German parallel corpus.
...
of improvement whereby multilingual outperform bilingual, which outperform monolingual grounded
...
Following \citep{kiros2014unifying} to predict images from the sentences
