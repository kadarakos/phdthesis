...
They introduce the idea of jointly learning many linguistic tasks at the same time such as part-of-speech
...
We also make extensive use of the  learning strategy: 
 Chapters~\ref{ch:COLI} for
...
take a variable length sequence as input and perform a stateful computation over its elements,
...
\begin{align}
P(w_n|w_{n<n}) &= \text{softmax}(\mathbf{U} \mathbf{h_t} + \mathbf{b_o}) \\
...
is a pair  with text document  and image.
...
Each document then represented as the concatenation of BoW textual an BoVW visual features.  
and a Latent Dirichlet Allocation \citep{blei2003latent} topic model is trained
...
to combine the two modalities.\footnote{Discovering latent factors through the application
...
Furthermore, in \cite{bruni2014multimodal} and \cite{kiela2014learning} textual and visual
...
As each word embedding $\mathbf{W_i}$ is represented in the
...
Following \cite{kiros2014unifying} to predict images from the sentences
...
specific networks and are applied to extract non-linear features
...
work on the topic from \citep{hodosh2013framing}
...
The first approach to construct a dictionary based on image similarity \cite{bergsma2011learning} 
